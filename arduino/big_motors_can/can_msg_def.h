...
enum class CAN_ID { ESTOP = 0x000, MOTOR = 0x100 };
...
public:
  ...
  MotorMsg(uint8_t left, uint8_t right)
      : left_speed(left), right_speed(right), buf_len(8) {
    buf = new uint8_t[buf_len];
    buf[0] = left_speed;
    buf[1] = right_speed;
  ...
  // Copy constructor
  MotorMsg(const MotorMsg &other)
      : left_speed(other.left_speed), right_speed(other.right_speed),
